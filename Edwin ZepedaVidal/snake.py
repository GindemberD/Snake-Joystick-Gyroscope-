...
serialDevFile = 'com5'
arduinoSerial = serial.Serial(serialDevFile, 9600, timeout=2)
...
while True:
    ...
    # elif ......
    #
    #Arduino serial communication to Python
   

    data=str(arduinoSerial.readline().decode('ascii'))
    
    (xValue,yValue)=data.split(',')
    
    print(xValue)
    print(yValue)

#JoyStick direction-inputs
...
#Gyro 6050 direction-inputs
    if (yValue.find("left") != -1): 
        head.direction = "left"
    elif (yValue.find("right") != -1):
        head.direction = "right"
    elif (xValue.find("up") != -1):
        head.direction = "up" 
    elif (xValue.find("down") != -1):
        head.direction = "down"
    ...
    if head.distance(food) < 20:
        ...
        arduinoSerial.writelines(b'H')   # send a byte
        time.sleep(1)        # wait 0.5 seconds
        arduinoSerial.writelines(b'L')   # send a byte
        time.sleep(0.5)
